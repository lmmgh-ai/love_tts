...
public:
    ...
    TextSet(const char *path)
    {
        ...
        { // 分割中英文
            while (idx < str.length() && (((uint8_t)str[idx]) >= 128 || isdigit(str[idx]) || str[idx] == ' '))
                idx++;
            if (idx > idxLast)
            {
            ...
            while (idx < str.length() && ((uint8_t)str[idx]) < 128 && !isdigit(str[idx]))
                idx++;
            if (idx > idxLast)
            {
                auto tmp = Trim(str.substr(idxLast, idx - idxLast));
    ...
    TextSet(const char *path, int a)
    {
        struct stat st;

        auto str = preProcess(path); // 预处理：只保留汉字，英文，数字，替换部分符号
    ...
    std::string preProcess(std::string &&str)
    {
        ...
        while (i < len)
        {
            ...
            else if (0x4e00 <= codePoint && codePoint <= 0x9fff)
            { // 中文区
