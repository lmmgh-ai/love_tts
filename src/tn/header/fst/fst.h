  ...
  protected:
    bool WriteFile(const string &filename) const
    {
      if (!filename.empty())
      {
        ...
        if (!val)
          LOG(ERROR) << "Fst::Write failed: " << filename;
        return val;
      }
      else
      {
  ...
  namespace internal
  {
    ...
    public:
      ...
      FstImpl &operator=(const FstImpl<Arc> &impl)
      {
        ...
        type_ = impl.type_;
        // isymbols_ = impl.isymbols_ ? impl.isymbols_->Copy() : nullptr;
        isymbols_ = impl.isymbols_ ? static_cast<decltype(isymbols_)>(impl.isymbols_->Copy()) : nullptr;
        //  osymbols_ = impl.osymbols_ ? impl.osymbols_->Copy() : nullptr;
        osymbols_ = impl.osymbols_ ? static_cast<decltype(osymbols_)>(impl.osymbols_->Copy()) : nullptr;
        return *this;
      }
